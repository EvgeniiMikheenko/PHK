...
uint16_t CalcCrc16Next(uint16_t oldCrc, uint16_t data, bool isInit) {
	register char CRC_Low, CRC_High;
	char carry;
	if(isInit) {
		CRC_Low = 0xFF;
		CRC_High = 0xFF;
...
uint16_t WriteCrc16(uint8_t *lpBuf, uint32_t bufSize) {
	uint32_t crcIndex = bufSize - 2;
	uint16_t crc = CalcCrc16(lpBuf, crcIndex);
	lpBuf[crcIndex++] = (unsigned int)crc;
	lpBuf[crcIndex] = (unsigned int)(crc >> 8);
	return crc;
}
...
bool TestCrc16(uint8_t *lpBuf, uint32_t bufSize) {
	uint32_t crcIndex = bufSize - 2;
	uint16_t crc = CalcCrc16(lpBuf, crcIndex);
	uint16_t *crcPacket = (uint16_t *)&lpBuf[crcIndex]; 
	if(*crcPacket != crc)
		return false;
	//
	return true;
...
//	uint16_t crc_tmp = CalcCrc16(lpBuf,bufSize);
//	
//	
//	
//	uint8_t *tmp = lpBuf+bufSize;
//	
//	tmp = &crc_tmp;
//	
//	
